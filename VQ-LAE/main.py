...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("-n", "--name", type=str, const=True, default="X4fisheye", nargs="?", help="postfix for logdir", )
    parser.add_argument("-r", "--resume", type=str, const=True, default="", nargs="?", help="resume from checkpoint", )     # Restart path
    parser.add_argument("-b", "--base", nargs="*", metavar="base_config.yaml", default=list(["configs/X4_fisheye.yaml"]),
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, batch_size_val, train=None, validation=None, test=None,
    ...
    def _val_dataloader(self):
        return DataLoader(self.datasets["validation"], batch_size=self.batch_size_val, num_workers=self.num_workers)

    def _test_dataloader(self):
        return DataLoader(self.datasets["test"], batch_size=self.batch_size_val, num_workers=self.num_workers)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if log:
            ...
            if split == 'val':
                ...
                HR = batch['HR']
                gts = ((HR[:N]+1.0)/2.0).mul(255).round().cpu()
...
################################     Keep the top 3 performing checkpoints   ################################
