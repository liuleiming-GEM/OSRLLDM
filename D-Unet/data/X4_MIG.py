...
class oditest(Dataset):
    def __init__(self, dictC):
        ...
        matrix_2048 = torch.zeros((2048, 4096))
        matimg_2048 = compute_map_ws(matrix_2048)
        matimg_2048 = matimg_2048.unsqueeze(0)

        self.matrix_128 = matimg_128
        self.matrix_256 = matimg_256
        self.matrix_512 = matimg_512
        self.matrix_1920 = matimg_1024
        self.matrix_2048 = matimg_2048
        self.input_LR_names = input_LR_names
        self.transforms = torchvision.transforms.Compose([torchvision.transforms.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        input_LR_img = PIL.Image.open(os.path.join(input_LR_names))
        lr_bic = cv2.resize(np.array(input_LR_img), (4096, 2048), interpolation=cv2.INTER_CUBIC)
        matrix_list = [self.matrix_256, self.matrix_512, self.matrix_1920, self.matrix_2048]
        lr = data_transform(self.transforms(input_LR_img))
