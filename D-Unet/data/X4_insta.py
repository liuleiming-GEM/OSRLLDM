...
def random_crops_tensor(img_tensor, y, x, h, w):
    new_crop = img_tensor[:, y:y + h, x:x + w]
    return new_crop
...
class oditra(Dataset):
    def __init__(self, dictC):
        ...
        images_BIC = [f for f in listdir(inputs_BIC)]
...
class oditest(Dataset):
    def __init__(self, dictC):
        ...
        inputs_LR = os.path.join(db_dir, )
        # inputs_LR = os.path.join(db_dir, 'HR')

        images_PNG = [f for f in listdir(inputs_LR)]
        ...
        matrix_1920 = torch.zeros((1920, 3840))
        matimg_1920 = compute_map_ws(matrix_1920)
        matimg_1920 = matimg_1920.unsqueeze(0)

        matrix_3840 = torch.zeros((3840, 7680))
        matimg_3840 = compute_map_ws(matrix_3840)
        matimg_3840 = matimg_3840.unsqueeze(0)

        matrix_7680 = torch.zeros((7680, 15360))
        matimg_7680 = compute_map_ws(matrix_7680)
        matimg_7680 = matimg_7680.unsqueeze(0)

        self.matrix_128 = matimg_128
        self.matrix_256 = matimg_256
        self.matrix_512 = matimg_512
        self.matrix_1920 = matimg_1920
        self.matrix_3840 = matimg_3840
        self.matrix_7680 = matimg_7680
        self.input_LR_names = input_LR_names
        self.transforms = torchvision.transforms.Compose([torchvision.transforms.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        input_LR_img = PIL.Image.open(os.path.join(input_LR_names))
        lr_bic = cv2.resize(np.array(input_LR_img), (15360, 7680), interpolation=cv2.INTER_CUBIC)
        matrix_list = [self.matrix_128, self.matrix_1920, self.matrix_3840, self.matrix_7680]
        lr = data_transform(self.transforms(input_LR_img))
