...
class LAnet(nn.Module):
    ...
    def __init__(self,
        ...
        self.LSAB = LSAB(dim=embed_dim, num_heads=1)

        self.conv_before_upsample = nn.Sequential(
            nn.Conv2d(embed_dim, up_dim, 3, 1, 1), nn.SiLU(inplace=True))

        # 2
        if upscale==2:
            self.upsample2 = Upsample(2, up_dim)
            self.after_upsample2 = nn.Sequential(
                nn.Conv2d(up_dim, 64, 3, 1, 1), nn.SiLU(inplace=True))

        # 4
        elif upscale == 4:
            self.upsample2 = Upsample(2, up_dim)
            self.after_upsample2 = nn.Sequential(
                nn.Conv2d(up_dim, up_dim, 3, 1, 1), nn.SiLU(inplace=True))
            self.upsample4 = Upsample(4, up_dim)
        ...
        # 8
            ...
            self.after_upsample4 = nn.Sequential(
                nn.Conv2d(up_dim, up_dim, 3, 1, 1), nn.SiLU(inplace=True))
            self.upsample8 = Upsample(8, up_dim)
    ...
    def forward(self, x, condlr, scale):
        x = self.conv_first(x)
        _x = self.conv_after_body(self.forward_features(x, condlr))
        x = self.LSAB(self.dcn(_x , self.offset_conv(condlr))) + x
        x1 = self.conv_before_upsample(x)

        if scale == 2:
            x2 = self.after_upsample2(self.upsample2(x1))
            return x1, x2

        elif scale == 4:
            x2 = self.after_upsample2(self.upsample2(x1))
            x4 = self.after_upsample4(self.upsample4(x1))
            return x1, x2, x4

        elif scale == 8:
            x2 = self.after_upsample2(self.upsample2(x1))
            x4 = self.after_upsample4(self.upsample4(x1))
            x8 = self.after_upsample8(self.upsample8(x1))
            return x2, x4, x8
